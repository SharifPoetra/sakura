...
exports.run = async (client, msg, args) => {
	...
	if (!args[1]) return msg.channel.send({ embed: { color: 0xFF0000, description: `*Correct usage is*: **${prefix}search** ***[Song Name]***`}});
		...
		try {
			...
			try {
				...
				if((/all/gi).test(response.first().content)){
					...
					return msg.channel.send({ embed: { color: 0x008000, description: `✅ Added 10 songs with query **${searchString}**`}});
				}
