...
exports.run = async (client, msg, args) => {
  ...
  if (!args[1])
    return msg.channel.send({
      embed: {
        color: 0xff0000,
        description: `*Correct usage is*: **${prefix}search** ***[Song Name]***`
      }
    });
    ...
    try {
      ...
      try {
        ...
        var selectembed = new MessageEmbed()
          ...
          .setFooter(
            "Please provide a value to select one of the search results ranging from 1-10 or all"
          );

        let msgtoDelete = await msg.channel.send({ embed: selectembed });
        try {
          const filter = msg2 =>
            ((msg2.content > 0 && msg2.content < 11) ||
              /all/gi.test(msg2.content)) &&
        ...
        if (/all/gi.test(response.first().content)) {
          ...
          return msg.channel.send({
            embed: {
              color: 0x008000,
              description: `✅ Added 10 songs with query **${searchString}**`
            }
          });
        }
