...
exports.run = (client, message, args, color) => {
  ...
  if (!args.join(" "))
    return message.channel.send("No parameter to execute. you're stuppid");
  const mu = Date.now();
  let command = `\`\`\`bash\n${args.slice(1).join(" ")}\`\`\``;
...
exports.help = {
  name: "exec",
  description:
    "Executes a command in the Terminal (Linux/macOS) or Command Prompt (Windows) and shows the output",
  usage: "exec <args>",
